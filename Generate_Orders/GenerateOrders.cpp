...
void GenerateOrders::addLimit()
{
    ...
    if (buyOrSell)
    {
        ...
        } while (limitPrice >= book->getLowestSell()->get_limitPrice());  
    } else {
        do {
            limitPrice = limitPriceDist(gen);
        } while (limitPrice <= book->getHighestBuy()->get_limitPrice());  
    }
...
void GenerateOrders::cancelLimit()
{
    ...
    int orderId = order->get_idNumber();
...
{
    std::uniform_int_distribution<> sharesDist(1, 1000);
    std::normal_distribution<> limitPriceDist(book->getHighestBuy()->get_limitPrice(), 50);
    ...
    int orderId = order->get_idNumber();
    bool buyOrSell = order->get_buyorsell();
    int limitPrice;
    if (buyOrSell)
    {
        do {
...
void GenerateOrders::addLimitMarket()
{
    ...
    if (buyOrSell)
    {
        limitPrice = book->getLowestSell()->get_limitPrice() + 1;
    } else {
        limitPrice = book->getHighestBuy()->get_limitPrice() - 1;
    }
...
{
    std::uniform_int_distribution<> sharesDist(1, 1000);
    std::normal_distribution<> stopPriceDist(book->getHighestBuy()->get_limitPrice(), 50);
    std::uniform_int_distribution<> buyOrSellDist(0, 1);
    ...
    if (buyOrSell)
    {
        ...
        } while (stopPrice <= book->getLowestSell()->get_limitPrice());  
    } else {
        do {
            stopPrice = stopPriceDist(gen);
        } while (stopPrice >= book->getHighestBuy()->get_limitPrice());  
    }
